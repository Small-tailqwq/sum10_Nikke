...
- `auto_capture.py` - 自动截图工具（日常使用，包含核心透视变换逻辑）
...
```python
from eyes.auto_capture import load_coordinates, unwarp_board
...
目前 `unwarp_board` 函数会自动根据四个角点的距离计算最佳尺寸。
如果需要固定尺寸，请修改 `eyes/auto_capture.py` 中的 `unwarp_board` 函数。

### 💡 关于尺寸与识别的说明
本项目采用 **相对切割 + 强制归一化** 的策略：
1. **相对切割**：无论截图大小，都会按 16x10 的比例切分网格。
2. **强制归一化**：切分后的每个数字图片，在送入 CNN 模型前都会被强制缩放为 **64x64** 像素。
因此，只要截图清晰且比例正确，**尺寸大小不会影响识别准确率**。
...
├── auto_capture.py             # 自动截图工具 (含核心变换逻辑)
